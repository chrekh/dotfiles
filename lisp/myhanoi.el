...
(defun hanoi (nrings)
  ...
  (let* (floor-row
	 ...
	 (pole-spacing (logand (/ window-width 8) (lognot 1))))
    (let (
	  ...
	  (progn
	    ...
	    (if (not (and (>= (setq window-height
			      ...
			      h)
			  (> (setq pole-spacing
				   (logand (/ window-width 8) (lognot 1)))
			     w)))
    ...
    (let ((fly-row (- floor-row nrings 1))
	  ...
	  (pole-4 (cons (* 7 pole-spacing) floor-row))
	...
	(while (< i nrings)
      ...
      ;; do it!
      ;;
      (hanoi0 (1- nrings) pole-1 pole-2 pole-3 pole-4)
      (goto-char (point-min))
      (message "Done")
      (setq buffer-read-only t)
...
;;;
(defun hanoi0 (n from to work1 work2)
  (cond ((input-pending-p)
	 (signal 'quit (list "I can tell you've had enough")))
	((< n 0))
	((= n 0)
	 (hanoi-move-ring 0 from to))
	(t
	 (hanoi0 (- n 1) from work1 to work2)
	 (hanoi-move-ring 1 from to)
	 (hanoi0 (- n 1) work1 to from work2))))
